...
print("Loading and aggregating NDJSON files (AS) on-the-fly...")
        ...
        with f_obj as f:
            for line in f:
                ...
                try:
                    ...
                    key = (prov, zs, ant, as_, ym)
                    if key not in agg_as:
                        agg_as[key] = {"n":0, "rap":0, "sum_comp":0.0, "sum_prompt":0.0}
                    g = agg_as[key]
                    g["n"] += 1
...
agg_zs = {}
agg_prov = {}
for key, g in agg_as.items():
    prov, zs, ant, as_, ym = key
    update_agg_from_agg(agg_zs, (prov, zs, ant, ym), g)
...
def write_split(base_dir: Path, records: list[dict]):
    base_dir.mkdir(parents=True, exist_ok=True)
    by_key = defaultdict(list)
    for r in records: by_key[r.get("_Province", "") or "unknown"].append(r)
    manifest = {}
